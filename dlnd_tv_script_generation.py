...
def create_lookup_tables(text):
    ...
    int_to_vocab = {index: word for index,word in enumerate(sorted_words)}
...
def token_lookup():
    ...
    """
    # TODO: Implement Function
    punct_to_token = {'.':'||Period||', ',':'||Comma||', '"':'||QuotationMark||', ';':'||Semicolon||', 
...
def get_batches(int_text, batch_size, seq_length):
    ...
    arr = np.array(int_text[:n_batches*chars_per_batch])
    arry = np.array(int_text[1:n_batches*chars_per_batch+1])
...
def pick_word(probabilities, int_to_vocab):
    ...
    """
    # TODO: Implement Function
    max_index = np.max(probabilities)
    loc = np.where(probabilities == max_index)
    word = int_to_vocab[loc[0][0]]
    return word
